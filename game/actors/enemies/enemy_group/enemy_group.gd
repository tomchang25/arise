...
func register_member(enemy: Enemy) -> void:
	...
	# EnemyGroup is the single source of truth for each member's anchor_position.
	# The enemy's spawn position was already set via enemy.anchor_position before
	# add_child(), so we simply confirm it here for clarity.
	# (No dynamic updates needed for stationary enemies; override _physics_process
	#  in a subclass if the anchor should move at runtime.)
