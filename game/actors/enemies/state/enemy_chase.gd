...
## How far the enemy can wander from anchor_position before giving up the chase.
...
func _enter() -> void:
	enemy.play_animation(Actor.ANIM_MOVE, 1.5)
...
func _update(_delta: float) -> void:
	...
	# Exit: leash.
	if enemy.get_distance_to_anchor() > leash_distance:
	...
	if target:
		...
		enemy.set_facing_direction(enemy.global_position.direction_to(target.global_position), Actor.ANIM_MOVE)
