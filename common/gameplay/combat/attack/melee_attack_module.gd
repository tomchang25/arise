...
func _execute_attack_logic(target_position: Vector2, data: AttackData) -> void:
    ...
    if not is_instance_valid(spawn_parent):
        push_error("MeleeAttackModule: could not resolve a valid spawn parent")
        end_attack()
        return
    ...
    if not result.success:
        push_error("MeleeAttackModule: spawn failed — %s" % result.blocked_reason)
        end_attack()
        return

    var delivery := result.spawned_node as AttackDelivery
    if delivery == null:
        push_error("MeleeAttackModule: spawned node is not AttackDelivery")
        end_attack()
        return
