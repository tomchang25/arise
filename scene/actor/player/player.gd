...
func _initialize_components() -> void:
    if not stats:
        push_error("Player: Stats is not set")
        return
    ...
    if hurtbox:
        ...
        hurtbox.get_hit.connect(_on_player_get_hit)
...
func _on_player_get_hit(attack_info: AttackInfo) -> void:
    stats.health -= attack_info.damage
    print("Player took damage: ", attack_info.damage, " Remaining: ", stats.health)
...
func is_attack_triggered() -> bool:
    if not attack_module.can_attack():
        return false
...
func start_attack_logic(target_pos: Vector2) -> void:
    if attack_module and attack_module.can_attack():
